...
In the Try It Yourself sections don't be afraid to look through the already 
written code/syntax for hints and guidance. It's not cheating :) 
...
### Try it yourself!
create a dataset containing the mean, 1st quartile, 3rd quartile, and minimum 
of MMSE scores at each wave
hint quantile()
...
### Try it yourself!
Create a separate bar graph for each created variable using a loop (mean,  
1st quartile, 3rd quartile, and minimum ), use a different color for each graph
...
### Try it yourself!
Create a dataframe containing models which use the following covariates:
 age_basedate; age_basedate + sex; sex + education;age_basedate + sex + education
use the full dataset, i.e. do not break it up by sex
Note: these are the same covariates used in the previous Try It Yourself example
...
Optional, create the point estimates 
```{r}
